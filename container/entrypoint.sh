...
if [[ -n "${CONFIGURED_ACTIONS_RUNNER_FILES_DIR}" ]]; then
  ...
  if [[ -d "${CONFIGURED_ACTIONS_RUNNER_FILES_DIR}" ]]; then
    ...
    cp -p -r "${CONFIGURED_ACTIONS_RUNNER_FILES_DIR}/." "/home/runner"
  fi
...
if [[ -n "${CONFIGURED_ACTIONS_RUNNER_FILES_DIR}" ]]; then
  ...
  cp -p -r "/home/runner/_diag" "/home/runner/svc.sh" /home/runner/.[^.]* "${CONFIGURED_ACTIONS_RUNNER_FILES_DIR}"
fi
...
if [[ ${_RUN_AS_ROOT} == "true" ]]; then
  ...
  if [[ $(id -u) -eq 0 ]]; then
    ...
    /usr/bin/chown -R runner "${_RUNNER_WORKDIR}" /home/runner
